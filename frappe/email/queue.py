...
def send(
	...
	email_content = get_formatted_html(
		subject,
		message,
...
def prepare_message(email, recipient, recipients_list):
	...
	):  # is missing the check for unsubscribe message but will not add as there will be no unsubscribe url
	...
	if email.expose_recipients == "header":
		...
		if email.expose_recipients == "footer":
			...
			if email_sent_cc:
				email_sent_message = _("This email was sent to {0} and copied to {1}").format(
