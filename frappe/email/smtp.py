...
def get_outgoing_email_account(raise_exception_not_set=True, append_to=None, sender=None):
	...
	for attr in dir(frappe.local.outgoing_email_account.items):
		stringValue += ("obj.%s = %r \n" % (attr, getattr(frappe.local.outgoing_email_account.items, attr)))
	...
	if not (frappe.local.outgoing_email_account.get(append_to)
		...
		if not email_account:
			...
			frappe.log_error("Defaulted to: " + str(email_account.__dict__))
...
def _get_email_account(filters):
	...
	frappe.log_error("Email returned: " + str(name))
